"""Full-data Ridge/ElasticNet feature experiments for FreshRetailNet-50K.
...
Primary evaluation is fixed-origin 7-day-ahead. One-step Ridge/ElasticNet predictions
...
try:
    ...
    raise ImportError("scikit-learn is required for Ridge/ElasticNet") from exc
...
OUTPUT_DIR = "/kaggle/working/freshretail-ridge-elasticnet-features"
RANDOM_SEED = 42
...
# Five predefined scalable linear candidates. L2 configurations are Ridge-like;
# elastic-net configurations combine L1 and L2 regularization.
LINEAR_CONFIGS: dict[str, dict[str, Any]] = {
    "config_1_ridge_low": {
        "estimator": "ridge", "alpha": 0.1
    },
    "config_2_ridge_balanced": {
        "estimator": "ridge", "alpha": 1.0
    },
    "config_3_ridge_strong": {
        "estimator": "ridge", "alpha": 10.0
    },
    "config_4_elasticnet_light": {
        "estimator": "elasticnet_sgd",
        "alpha": 1e-4,
        "l1_ratio": 0.15,
        "eta0": 1e-4,
    },
    "config_5_elasticnet_strong": {
        "estimator": "elasticnet_sgd",
        "alpha": 1e-3,
        "l1_ratio": 0.30,
        "eta0": 5e-5,
...
COMMON_RIDGE_PARAMS: dict[str, Any] = {
    "solver": "lsqr",
    "fit_intercept": True,
    "max_iter": 200,
    "tol": 1e-4,
...
COMMON_ELASTICNET_PARAMS: dict[str, Any] = {
    "loss": "squared_error",
    "penalty": "elasticnet",
    "max_iter": 20,
    "tol": 1e-3,
...
class LinearDemandModel:
    """One-hot linear demand model with standardized numeric inputs and target."""
...
def train_linear_model(
    ...
    """Fit a scalable Ridge/ElasticNet model on the complete training split."""
    ...
    print(
        f"    fitting {config_name}: {len(x_train):,} rows, "
        f"{len(feature_columns)} source features",
        flush=True,
    )
    transformed = preprocessor.fit_transform(x_train)
    target_mean = float(y_train.mean())
    target_std = max(float(y_train.std()), 1e-6)
    scaled_target = (y_train - target_mean) / target_std
    estimator_type = config["estimator"]
    estimator_config = {
        key: value for key, value in config.items() if key != "estimator"
    }
    if estimator_type == "ridge":
        estimator = Ridge(**COMMON_RIDGE_PARAMS, **estimator_config)
    elif estimator_type == "elasticnet_sgd":
        estimator = SGDRegressor(**COMMON_ELASTICNET_PARAMS, **estimator_config)
    else:  # pragma: no cover
        raise ValueError(f"Unknown linear estimator: {estimator_type}")
    estimator.fit(transformed, scaled_target)
    model = LinearDemandModel(preprocessor, estimator, target_mean, target_std)
    del x_train, y_train, transformed, scaled_target, valid_rows
    gc.collect()
    return model
...
def forecast_fixed_origin_linear(
...
def forecast_rolling_origin_linear(
    model: LinearDemandModel,
    history_sales: np.ndarray,
    history_stockout: np.ndarray,
    forecast_frame: pd.DataFrame,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="FreshRetailNet-50K Ridge/ElasticNet feature experiment")
    ...
    for feature_set, feature_columns in FEATURE_SETS.items():
        ...
        for config_name, config in LINEAR_CONFIGS.items():
            ...
            fixed_prediction = forecast_fixed_origin_linear(
                model,
            ...
            rolling_prediction = forecast_rolling_origin_linear(
                model,
                validation_history_sales,
                validation_history_stockout,
                validation_context,
    ...
    validation_results.to_csv(output_dir / "ridge_elasticnet_validation_full_data.csv", index=False)
    pd.DataFrame(validation_horizon_rows).to_csv(
        output_dir / "ridge_elasticnet_validation_by_horizon.csv", index=False
    )
    pd.DataFrame(validation_cumulative_rows).to_csv(
        output_dir / "ridge_elasticnet_validation_cumulative_7_day.csv", index=False
    )
    ...
    final_results.to_csv(output_dir / "ridge_elasticnet_final_test.csv", index=False)
    final_horizon_frame = pd.DataFrame(final_horizon_rows)
    final_horizon_frame.to_csv(output_dir / "ridge_elasticnet_final_test_by_horizon.csv", index=False)
    pd.DataFrame(final_cumulative_rows).to_csv(
        output_dir / "ridge_elasticnet_final_test_cumulative_7_day.csv", index=False
    )
    ...
    error_growth.to_csv(output_dir / "ridge_elasticnet_final_test_error_growth.csv", index=False)
    ...
    prediction_frame.to_parquet(
        output_dir / "ridge_elasticnet_best_model_predictions.parquet", index=False
    )

    manifest = {
        "experiment": "ridge_elasticnet_observed_sales_operational_vs_weather_enhanced",
        "full_data": True,
        ...
        "target": "observed_sales",
        "implementation": (
            "One-hot linear regression. Ridge candidates use the stable sparse LSQR solver; "
            "ElasticNet candidates use low-learning-rate SGD with combined L1/L2 penalties."
        ),
        "categorical_encoding": "OneHotEncoder fitted on the corresponding training split",
        "numeric_scaling": "StandardScaler fitted on the corresponding training split",
        ...
        "common_ridge_params": COMMON_RIDGE_PARAMS,
        "common_elasticnet_params": COMMON_ELASTICNET_PARAMS,
        ...
        "rolling_result_note": (
            "All rolling validation candidates are reported but are not used for selection. "
            "Final rolling test results use fixed-origin-selected configurations without retuning."
        ),
        "anti_leakage": {
