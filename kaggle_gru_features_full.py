"""Full-data GRU forecasting experiments for FreshRetailNet-50K.
...
The GRU uses 28 historical days and an autoregressive seven-step decoder. It
...
try:
    ...
    raise ImportError("PyTorch is required for the GRU experiment") from exc
...
OUTPUT_DIR = "/kaggle/working/freshretail-gru-features"
RANDOM_SEED = 42
HISTORY_LENGTH = 28
FORECAST_HORIZON = 7
...
GRU_CONFIGS: dict[str, dict[str, Any]] = {
    "config_1_fast": {
        "hidden_size": 64,
        "num_layers": 1,
        "dropout": 0.0,
...
def select_device() -> torch.device:
    ...
    if not torch.cuda.is_available():
        print("WARNING: CUDA is unavailable; full-data GRU training will be very slow.", flush=True)
...
def load_data(train_path: Path, eval_path: Path) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any]]:
    print("Loading GRU columns from train.parquet ...", flush=True)
    train = pd.read_parquet(train_path, columns=REQUIRED_COLUMNS)
    print("Loading GRU columns from eval.parquet ...", flush=True)
...
class GRUForecaster(nn.Module):
    def __init__(
        ...
        self.encoder = nn.GRU(
            input_size=2 + context_size + static_size,
        ...
        self.decoder = nn.GRUCell(
            input_size=1 + context_size + static_size,
            hidden_size=hidden_size,
        )
    ...
    def forward(
        ...
        for step in range(future_context.shape[1]):
            decoder_input = torch.cat(
                [previous_sale.unsqueeze(1), future_context[:, step], static], dim=1
            )
            decoder_hidden = self.decoder(decoder_input, decoder_hidden)
            prediction = F.softplus(self.output(decoder_hidden).squeeze(1))
...
def train_gru(
    ...
    config: dict[str, Any],
    device: torch.device,
) -> tuple[GRUForecaster, np.ndarray, list[float]]:
    ...
    if not len(origins):
        raise ValueError("Insufficient dates for GRU training windows")
...
def forecast_rolling_origin_gru(
    model: GRUForecaster,
    history_sales: np.ndarray,
    history_stockout: np.ndarray,
    history_context: np.ndarray,
    ...
    for step in range(future_sales.shape[1]):
        predictions[:, step : step + 1] = forecast_gru(
            model,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="FreshRetailNet-50K GRU experiment")
    ...
    if device.type != "cuda":
        print("WARNING: GRU full-data training is much faster with a Kaggle GPU.", flush=True)
    ...
    for feature_set in FEATURE_SETS:
        ...
        for config_name, config in GRU_CONFIGS.items():
            model, series_scale, losses = train_gru(
                train_sales,
                train_stockout,
                context,
                static_codes,
            ...
            fixed = forecast_fixed_origin_gru(
                model,
            ...
            rolling = forecast_rolling_origin_gru(
                model,
                train_sales[:, :validation_index],
                train_stockout[:, :validation_index],
                context[:, :validation_index],
    ...
    validation_results.to_csv(output_dir / "gru_validation_full_data.csv", index=False)
    pd.DataFrame(validation_horizon_rows).to_csv(
        output_dir / "gru_validation_by_horizon.csv", index=False
    )
    pd.DataFrame(validation_cumulative_rows).to_csv(
        output_dir / "gru_validation_cumulative_7_day.csv", index=False
    )
    ...
    final_results.to_csv(output_dir / "gru_final_test.csv", index=False)
    final_horizon_frame = pd.DataFrame(final_horizon_rows)
    final_horizon_frame.to_csv(output_dir / "gru_final_test_by_horizon.csv", index=False)
    pd.DataFrame(final_cumulative_rows).to_csv(
        output_dir / "gru_final_test_cumulative_7_day.csv", index=False
    )
    ...
    error_growth.to_csv(output_dir / "gru_final_test_error_growth.csv", index=False)
    ...
    prediction_frame.to_parquet(output_dir / "gru_best_model_predictions.parquet", index=False)

    manifest = {
        "experiment": "gru_observed_sales_operational_vs_weather_enhanced",
        "full_data": True,
        ...
        "architecture": "GRU encoder with autoregressive GRUCell decoder",
