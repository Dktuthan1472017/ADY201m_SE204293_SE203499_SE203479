"""Full-data XGBoost experiments for FreshRetailNet-50K.

Paste this file into one Kaggle notebook cell after attaching exactly one
train.parquet and one eval.parquet below /kaggle/input. This standalone script trains XGBoost only.
...
try:
    ...
    raise ImportError("XGBoost is required. In Kaggle run: !pip install xgboost") from exc
...
OUTPUT_DIR = "/kaggle/working/freshretail-xgboost-features"
...
def train_model(
    ...
    if model_family != "xgboost":  # pragma: no cover
...
def main() -> None:
    parser = argparse.ArgumentParser(description="FreshRetail XGBoost full experiment")
    ...
    manifest = {
        "experiment": "xgboost_observed_sales_feature_comparison",
