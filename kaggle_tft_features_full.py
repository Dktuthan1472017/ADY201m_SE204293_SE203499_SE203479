"""Full-data TFT forecasting experiments for FreshRetailNet-50K.
...
The TFT uses 28 historical days and an autoregressive seven-step decoder. It
...
try:
    ...
    raise ImportError("PyTorch is required for the TFT experiment") from exc
...
OUTPUT_DIR = "/kaggle/working/freshretail-tft-features"
RANDOM_SEED = 42
HISTORY_LENGTH = 28
FORECAST_HORIZON = 7
...
TFT_CONFIGS: dict[str, dict[str, Any]] = {
    "config_1_fast": {
        "hidden_size": 64,
        "num_layers": 1,
        "num_heads": 4,
...
def select_device() -> torch.device:
    ...
    if not torch.cuda.is_available():
        print("WARNING: CUDA is unavailable; full-data TFT training will be very slow.", flush=True)
...
def load_data(train_path: Path, eval_path: Path) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any]]:
    print("Loading TFT columns from train.parquet ...", flush=True)
    train = pd.read_parquet(train_path, columns=REQUIRED_COLUMNS)
    print("Loading TFT columns from eval.parquet ...", flush=True)
...
class VariableSelectionNetwork(nn.Module):
    def __init__(self, variable_count: int, hidden_size: int, dropout: float) -> None:
        super().__init__()
    ...
    def forward(self, values: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        weights = torch.softmax(self.weight_network(values), dim=-1)
        embedded = torch.stack(
            [layer(values[..., index : index + 1]) for index, layer in enumerate(self.variable_embeddings)],
            dim=-2,
        )
...
class TFTForecaster(nn.Module):
    def __init__(
        ...
        super().__init__()
        embedding_dims = [min(16, max(2, int(math.ceil(math.sqrt(size))))) for size in cardinalities]
        self.embeddings = nn.ModuleList(
            [nn.Embedding(size, dim) for size, dim in zip(cardinalities, embedding_dims, strict=True)]
        )
        static_size = sum(embedding_dims)
        self.num_layers = num_layers
        self.history_selection = VariableSelectionNetwork(2 + context_size, hidden_size, dropout)
        self.future_selection = VariableSelectionNetwork(context_size, hidden_size, dropout)
        self.static_projection = nn.Linear(static_size, hidden_size)
        self.static_hidden = nn.Linear(static_size, hidden_size)
        self.static_cell = nn.Linear(static_size, hidden_size)
    ...
    def forward(
        ...
        history_variables = torch.cat(
            [history_sales.unsqueeze(-1), history_stockout.unsqueeze(-1), history_context],
            dim=2,
        )
        selected_history, _ = self.history_selection(history_variables)
        selected_future, _ = self.future_selection(future_context)
        static_context = self.static_projection(static).unsqueeze(1)
        selected_history = selected_history + static_context
        selected_future = selected_future + static_context
        initial_hidden = torch.tanh(self.static_hidden(static)).unsqueeze(0).repeat(
            self.num_layers, 1, 1
        )
        initial_cell = torch.tanh(self.static_cell(static)).unsqueeze(0).repeat(
            self.num_layers, 1, 1
        )
        encoded, state = self.encoder(
            selected_history, (initial_hidden, initial_cell)
        )
        decoded, _ = self.decoder(selected_future, state)
        attended, _ = self.attention(decoded, encoded, encoded, need_weights=False)
        gate = torch.sigmoid(self.gate(torch.cat([decoded, attended], dim=-1)))
        fused = self.norm(decoded + gate * attended)
        return F.softplus(self.output(fused).squeeze(-1))
...
def train_tft(
    ...
    config: dict[str, Any],
    device: torch.device,
) -> tuple[TFTForecaster, np.ndarray, list[float]]:
    ...
    if not len(origins):
        raise ValueError("Insufficient dates for TFT training windows")
...
def forecast_rolling_origin_tft(
    model: TFTForecaster,
    history_sales: np.ndarray,
    history_stockout: np.ndarray,
    history_context: np.ndarray,
    ...
    for step in range(future_sales.shape[1]):
        predictions[:, step : step + 1] = forecast_tft(
            model,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="FreshRetailNet-50K TFT experiment")
    ...
    if device.type != "cuda":
        print("WARNING: TFT full-data training is much faster with a Kaggle GPU.", flush=True)
    ...
    for feature_set in FEATURE_SETS:
        ...
        for config_name, config in TFT_CONFIGS.items():
            model, series_scale, losses = train_tft(
                train_sales,
                train_stockout,
                context,
                static_codes,
            ...
            fixed = forecast_fixed_origin_tft(
                model,
            ...
            rolling = forecast_rolling_origin_tft(
                model,
                train_sales[:, :validation_index],
                train_stockout[:, :validation_index],
                context[:, :validation_index],
    ...
    validation_results.to_csv(output_dir / "tft_validation_full_data.csv", index=False)
    pd.DataFrame(validation_horizon_rows).to_csv(
        output_dir / "tft_validation_by_horizon.csv", index=False
    )
    pd.DataFrame(validation_cumulative_rows).to_csv(
        output_dir / "tft_validation_cumulative_7_day.csv", index=False
    )
    ...
    for feature_set in FEATURE_SETS:
        ...
        config = TFT_CONFIGS[config_name]
    ...
    final_results.to_csv(output_dir / "tft_final_test.csv", index=False)
    final_horizon_frame = pd.DataFrame(final_horizon_rows)
    final_horizon_frame.to_csv(output_dir / "tft_final_test_by_horizon.csv", index=False)
    pd.DataFrame(final_cumulative_rows).to_csv(
        output_dir / "tft_final_test_cumulative_7_day.csv", index=False
    )
    ...
    error_growth.to_csv(output_dir / "tft_final_test_error_growth.csv", index=False)
    ...
    prediction_frame.to_parquet(output_dir / "tft_best_model_predictions.parquet", index=False)

    manifest = {
        "experiment": "tft_observed_sales_operational_vs_weather_enhanced",
        "full_data": True,
        ...
        "target": "observed_sales",
        "implementation": "custom self-contained PyTorch TFT-style implementation",
        "history_length": HISTORY_LENGTH,
        "forecast_horizon": FORECAST_HORIZON,
        "architecture": (
            "TFT-style point forecaster with per-variable selection, static enrichment, "
            "LSTM encoder/decoder, gated multi-head attention, and direct multi-horizon output"
        ),
        ...
        "anti_leakage": {
            "fixed_future_sales": "autoregressive predictions only",
