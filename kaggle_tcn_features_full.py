"""Full-data TCN forecasting experiments for FreshRetailNet-50K.
...
The TCN uses 28 historical days and an autoregressive seven-step decoder. It
...
try:
    ...
    raise ImportError("PyTorch is required for the TCN experiment") from exc
...
OUTPUT_DIR = "/kaggle/working/freshretail-tcn-features"
RANDOM_SEED = 42
HISTORY_LENGTH = 28
FORECAST_HORIZON = 7
...
TCN_CONFIGS: dict[str, dict[str, Any]] = {
    "config_1_fast": {
        "channels": 64,
        "levels": 3,
        "kernel_size": 3,
...
def select_device() -> torch.device:
    ...
    if not torch.cuda.is_available():
        print("WARNING: CUDA is unavailable; full-data TCN training will be very slow.", flush=True)
...
def load_data(train_path: Path, eval_path: Path) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any]]:
    print("Loading TCN columns from train.parquet ...", flush=True)
    train = pd.read_parquet(train_path, columns=REQUIRED_COLUMNS)
    print("Loading TCN columns from eval.parquet ...", flush=True)
...
class CausalTCNBlock(nn.Module):
    ...
    def _causal(self, values: torch.Tensor) -> torch.Tensor:
        return values[:, :, :-self.padding] if self.padding else values

    def forward(self, values: torch.Tensor) -> torch.Tensor:
        residual = self.residual(values)
        output = self.dropout(F.gelu(self._causal(self.conv1(values))))
...
class TCNForecaster(nn.Module):
    def __init__(
        ...
        input_channels = 2 + context_size + static_size
        self.network = nn.Sequential(
            *[
                CausalTCNBlock(
                    input_channels if level == 0 else channels,
                    channels,
                    kernel_size,
                    2 ** level,
                    dropout,
        ...
        self.decoder = nn.Sequential(
            nn.Linear(channels + 1 + context_size + static_size, channels),
            nn.GELU(),
            nn.Dropout(dropout),
    ...
    def forward(
        ...
        repeated_static = static.unsqueeze(1).expand(-1, history_sales.shape[1], -1)
        sequence = torch.cat(
            [history_sales.unsqueeze(-1), history_stockout.unsqueeze(-1), history_context, repeated_static],
            dim=2,
        ).transpose(1, 2)
        encoded = self.network(sequence)[:, :, -1]
        previous_sale = history_sales[:, -1]
        predictions = []
        for step in range(future_context.shape[1]):
            decoder_input = torch.cat(
                [encoded, previous_sale.unsqueeze(1), future_context[:, step], static], dim=1
            )
            prediction = F.softplus(self.decoder(decoder_input).squeeze(1))
            predictions.append(prediction)
            previous_sale = prediction
...
def train_tcn(
    ...
    config: dict[str, Any],
    device: torch.device,
) -> tuple[TCNForecaster, np.ndarray, list[float]]:
    ...
    if not len(origins):
        raise ValueError("Insufficient dates for TCN training windows")
...
def forecast_rolling_origin_tcn(
    model: TCNForecaster,
    history_sales: np.ndarray,
    history_stockout: np.ndarray,
    history_context: np.ndarray,
    ...
    for step in range(future_sales.shape[1]):
        predictions[:, step : step + 1] = forecast_tcn(
            model,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="FreshRetailNet-50K TCN experiment")
    ...
    if device.type != "cuda":
        print("WARNING: TCN full-data training is much faster with a Kaggle GPU.", flush=True)
    ...
    for feature_set in FEATURE_SETS:
        ...
        for config_name, config in TCN_CONFIGS.items():
            model, series_scale, losses = train_tcn(
                train_sales,
                train_stockout,
                context,
                static_codes,
            ...
            fixed = forecast_fixed_origin_tcn(
                model,
            ...
            rolling = forecast_rolling_origin_tcn(
                model,
                train_sales[:, :validation_index],
                train_stockout[:, :validation_index],
                context[:, :validation_index],
    ...
    validation_results.to_csv(output_dir / "tcn_validation_full_data.csv", index=False)
    pd.DataFrame(validation_horizon_rows).to_csv(
        output_dir / "tcn_validation_by_horizon.csv", index=False
    )
    pd.DataFrame(validation_cumulative_rows).to_csv(
        output_dir / "tcn_validation_cumulative_7_day.csv", index=False
    )
    ...
    for feature_set in FEATURE_SETS:
        ...
        config = TCN_CONFIGS[config_name]
    ...
    final_results.to_csv(output_dir / "tcn_final_test.csv", index=False)
    final_horizon_frame = pd.DataFrame(final_horizon_rows)
    final_horizon_frame.to_csv(output_dir / "tcn_final_test_by_horizon.csv", index=False)
    pd.DataFrame(final_cumulative_rows).to_csv(
        output_dir / "tcn_final_test_cumulative_7_day.csv", index=False
    )
    ...
    error_growth.to_csv(output_dir / "tcn_final_test_error_growth.csv", index=False)
    ...
    prediction_frame.to_parquet(output_dir / "tcn_best_model_predictions.parquet", index=False)

    manifest = {
        "experiment": "tcn_observed_sales_operational_vs_weather_enhanced",
        "full_data": True,
        ...
        "target": "observed_sales",
        "implementation": "custom self-contained PyTorch implementation",
        "history_length": HISTORY_LENGTH,
        "forecast_horizon": FORECAST_HORIZON,
        "architecture": "causal dilated TCN encoder with autoregressive MLP decoder",
